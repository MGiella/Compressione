...
def compressione(file_name: str, secret_key: str, mode: int):
    ...
    if using_blocks and len(stringInput) > block_length:
        ...
        block_length = math.ceil(len(stringInput) / size)
        data = comm.scatter([stringInput, block_length, MPI.CHAR], None, root=0)
        input_block = data + "\003" # Add EOF
        outputBWT_block = sbwt.bwt_from_suffix(input_block, r + secret_key)
        outputBWT = comm.gather(outputBWT_block, None, root=0)
